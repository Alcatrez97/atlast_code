...
import { Box, Container, Typography, Button, Card, CardContent, IconButton, TextField, Chip, Divider, Tooltip, Grid, Paper } from '@mui/material';
...
export const RuleHelpPage = ({ onShowNotification }) => {
    ...
    const [sandboxExpression, setSandboxExpression] = useState("context.amount > 1000 and context.customer?.tier == 'VIP'");
    const [loading, setLoading] = useState(false);
    const [evalResult, setEvalResult] = useState(null);
    const handleRunEvaluation = async (exprOverride, ctxOverride) => {
        setLoading(true);
        setEvalResult(null);
        try {
    ...
    const loadExampleIntoSandbox = (example) => {
    ...
    const categories = [
        {
            ...
            description: "How to reference fields in the workflow context. Standard fields can be accessed directly.",
            examples: [
                {
        ...
        <Grid container spacing={4}>
          ...
          <Grid size={{ xs: 12, lg: 6 }}>
              ...
              <CardContent sx={{ p: 3, flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                ...
                <Grid container spacing={2} sx={{ flexGrow: 1 }}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    ...
                    <TextField multiline rows={14} fullWidth value={sandboxContext} onChange={(e) => setSandboxContext(e.target.value)} sx={{
            '& .MuiInputBase-root': {
                  ...
                  <Grid size={{ xs: 12, sm: 6 }} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <Box>
                        ...
                        <TextField fullWidth value={sandboxExpression} onChange={(e) => setSandboxExpression(e.target.value)} placeholder="e.g. context.amount > 100" sx={{
            '& .MuiInputBase-root': {
                fontFamily: 'monospace',
                fontSize: '13px',
                bgcolor: 'rgba(0,0,0,0.2)',
                ...
                borderRadius: 2
                      ...
                      <Button variant="contained" onClick={() => handleRunEvaluation()} disabled={loading} startIcon={<PlayArrowIcon />} sx={{
            ...
            py: 1, px: 3
                ...
                justifyContent: 'center'
                            ...
                            {evalResult.success ? (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Grid size={{ xs: 12, lg: 6 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {categories.map((cat, catIdx) => (<Card key={catIdx} sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }}>
                  <CardContent sx={{ p: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      {cat.examples.map((ex, exIdx) => (<Paper key={exIdx} sx={{
                          ...
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1 }}>
                              <Tooltip title="Try in Sandbox" arrow>
                                <Button variant="outlined" size="small" onClick={() => loadExampleIntoSandbox(ex)} sx={{
                    ...
                    '&:hover': { borderColor: accentColor, bgcolor: 'rgba(139,92,246,0.06)' }
                              ...
                              <Tooltip title="Copy Expression" arrow>
                                <IconButton size="small" onClick={() => handleCopyToClipboard(ex.expression)} sx={{ color: 'text.secondary', width: 22, height: 22 }}>
