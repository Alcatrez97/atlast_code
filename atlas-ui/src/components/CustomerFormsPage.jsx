...
import { Box, Container, Paper, Typography, Button, Chip, IconButton, InputBase, CircularProgress, TableContainer, Table, TableHead, TableBody, TableRow, TableCell, Alert, FormControl, Select, MenuItem, Grid } from '@mui/material';
...
const getOutcomeColor = (name) => {
    const n = name.toLowerCase();
    if (n.includes('accept') || n.includes('approve') || n.includes('success'))
        return 'success';
    if (n.includes('reject') || n.includes('deny') || n.includes('fail'))
        return 'error';
    if (n.includes('park') || n.includes('hold') || n.includes('defer'))
        return 'warning';
    return 'primary';
};
export const CustomerFormsPage = ({ onShowNotification }) => {
    ...
    const fetchForms = async (targetPage = page, targetSize = size, statusFilter = selectedStatus, searchTerm = search) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('Failed to fetch simulated customer forms');
    ...
    const [bucketOutcomesMap, setBucketOutcomesMap] = useState({});
    // Fetch all buckets on mount to get their possibleOutcomes
    useEffect(() => {
        fetch('/api/buckets')
            ...
            const outcomesMap = {};
            data.forEach(b => {
                outcomesMap[b.bucketId] = b.possibleOutcomes || [{ name: 'Accept' }, { name: 'Reject' }];
            });
            setBucketOutcomesMap(outcomesMap);
        })
            .catch(() => { });
    }, []);
    useEffect(() => {
    ...
    const handleUpdateStatus = async (id, currentStatus, outcomeName) => {
        ...
        // Find the mapped status from outcomes map or fall back
        const outcomes = bucketOutcomesMap[bucketId] || [{ name: 'Accept' }, { name: 'Reject' }];
        const matchedOutcome = outcomes.find(o => o.name.toLowerCase() === outcomeName.toLowerCase());
        const newStatus = matchedOutcome?.formStatus || `${bucketId}${outcomeName}`;
        setActioningId(id);
        try {
            const res = await fetch(`/api/forms/${id}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ status: newStatus, outcome: outcomeName })
            });
            if (!res.ok)
    ...
    return (<Box sx={{ bgcolor: 'background.default', minHeight: '92vh', py: 4, color: 'text.primary', transition: 'background-color 0.25s ease-in-out' }}>
      <Container maxWidth="lg">
        ...
        <Alert severity="info" sx={{
            bgcolor: 'rgba(12,173,239,0.05)',
            border: '1px solid rgba(12,173,239,0.15)',
            color: 'text.secondary',
            mb: 4,
            borderRadius: 2,
            '& .MuiAlert-icon': { color: '#0CADEF' }
        }}>
        ...
        <Paper sx={{
          ...
          <Grid container spacing={2} sx={{ alignItems: 'center' }}>
            ...
            bgcolor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 2
                ...
                <InputBase placeholder="Search by ID, Customer Name, or Status..." value={search} onChange={(e) => {
        ...
        {/* Content Table */}
        {loading && forms.length === 0 ? (<Box sx={{ display: 'flex', justifyContent: 'center', py: 12 }}>
            <CircularProgress color="secondary"/>
          </Box>) : forms.length === 0 ? (<Box sx={{
          ...
          </Box>) : (<TableContainer component={Paper} sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
            <Table>
              <TableHead>
              ...
              <TableBody>
                ...
                return (<TableRow key={form.id} sx={{
                      ...
                      <TableCell sx={{ border: 'none', textAlign: 'right' }}>
                        {isPending ? ((() => {
                        const bucketId = form.formStatus.split(' ')[0];
                        const outcomes = bucketOutcomesMap[bucketId] || [{ name: 'Accept' }, { name: 'Reject' }];
                        return (<Box sx={{ display: 'inline-flex', gap: 1 }}>
                                {outcomes.map(o => {
                                const color = getOutcomeColor(o.name);
                                return (<Button key={o.name} variant="contained" size="small" color={color} startIcon={isActioning ? <CircularProgress size={12} color="inherit"/> : (color === 'success' ? <CheckCircleIcon /> : <CancelIcon />)} onClick={() => handleUpdateStatus(form.id, form.formStatus, o.name)} disabled={isActioning} sx={{ textTransform: 'none', fontWeight: 700, px: 2, borderRadius: 1.5 }}>
                                      {o.name}
                                    </Button>);
                            })}
                              </Box>);
