...
export const WorkflowInstancesPage = ({ onShowNotification }) => {
    ...
    const fetchInstancesAndExecutions = async (targetPage = page, targetSize = size, wfKey = selectedWorkflowKey, status = selectedStatus, searchTerm = search) => {
        setLoading(true);
        try {
    ...
    const handleDeleteInstance = async (id, e) => {
        e.stopPropagation();
        if (!window.confirm('Are you sure you want to delete this stateful workflow instance?'))
            return;
        try {
    ...
    return (<Box sx={{ bgcolor: 'background.default', minHeight: '92vh', py: 4, color: 'text.primary', transition: 'background-color 0.25s ease-in-out, color 0.25s ease-in-out' }}>
