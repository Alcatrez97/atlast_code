...
export const NodePropertiesDrawer = ({ open, node, isReadOnly, traceStep, onClose, onSaveNode, onDeleteNode }) => {
    ...
    return (<Drawer anchor="right" open={open} onClose={onClose} variant="persistent" slotProps={{
      ...
      {node && (<Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          {/* Header */}
          <Box sx={{
          ...
          <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            {traceStep && (<Box>
                ...
                <Box sx={{
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                    ...
                    {traceStep.durationMs !== undefined && (<Typography variant="caption" color="text.secondary">{traceStep.durationMs}ms</Typography>)}
                  </Box>
            ...
            <Box>
              ...
              {/* WAIT_EVENT fields */}
              {nodeType === 'WAIT_EVENT' && (<>
                  <TextField fullWidth size="small" label="Event Type" value={eventType} onChange={(e) => setEventType(e.target.value)} disabled={isReadOnly} placeholder="BUCKET_COMPLETED" helperText={!isReadOnly ? "Correlation event name to wait for (e.g. BUCKET_COMPLETED)" : undefined} sx={{ mb: 1.5 }}/>

                  <TextField fullWidth size="small" label="Payload Routes (JSON Array)" value={routesStr} onChange={(e) => setRoutesStr(e.target.value)} disabled={isReadOnly} multiline rows={4} placeholder={`[\n  { "value": "APPROVED", "target": "NODE_APPROVED" },\n  { "value": "REJECTED", "target": "NODE_REJECTED" }\n]`} helperText={!isReadOnly ? "Map payload outcomes to target node IDs" : undefined} slotProps={{ input: { sx: { fontFamily: 'monospace', fontSize: '11px' } } }} sx={{ mb: 1.5 }}/>
