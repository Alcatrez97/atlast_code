...
import { Box, Typography, Button, Chip, Alert, IconButton, Tabs, Tab, TextField, InputAdornment, Tooltip, Select, MenuItem, FormControl, CircularProgress, Divider, Switch, FormControlLabel, } from '@mui/material';
...
export const ExecutionReplayPage = ({ onShowNotification }) => {
    ...
    if (!currentExecution || !replayVersion) {
        return (<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', flexDirection: 'column', gap: 2 }}>
    ...
    const fullTrace = currentExecution.executionTrace || [];
    const trace = debugView ? fullTrace : fullTrace.filter((s) => s.status !== 'SKIPPED');
    const currentStep = activeTraceStep !== null ? trace[activeTraceStep] : undefined;
    ...
    const formatValue = (val) => {
        ...
        if (typeof val === 'boolean')
            return <span style={{ color: val ? '#10b981' : '#ef4444', fontWeight: 600 }}>{val ? 'true' : 'false'}</span>;
        if (typeof val === 'number')
            return <span style={{ color: '#3b82f6' }}>{val}</span>;
        if (typeof val === 'object') {
            return (<pre style={{ margin: 0, fontSize: '11px', fontFamily: 'monospace', color: '#a78bfa', whiteSpace: 'pre-wrap' }}>
    ...
    return (<Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', gap: 2 }}>
          <FormControlLabel control={<Switch size="small" checked={debugView} onChange={(e) => {
                ...
                setActiveTraceStep(0);
            }}/>} label={<Typography variant="caption" sx={{ fontWeight: 600 }}>Debug View</Typography>}/>
          <Button size="small" variant="outlined" startIcon={<PlayCircleIcon />} onClick={() => setView('executor')} sx={{ borderColor: 'rgba(16,185,129,0.3)', color: '#10b981' }}>
            Run Again
          </Button>
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'flex', flexGrow: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{
          ...
          <Tabs value={tabValue} onChange={(_, val) => setTabValue(val)} variant="fullWidth" sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
          ...
          <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 1.5 }}>
            {tabValue === 0 && (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {trace.length === 0 ? (<Alert severity="info">No trace steps recorded.</Alert>) : (trace.map((step, idx) => {
                const isActive = idx === activeTraceStep;
                const statusColor = getStatusColor(step.status);
                return (<Box key={idx} onClick={() => handleStepClick(idx)} sx={{
                        p: 1.5,
                        cursor: 'pointer',
                        borderRadius: '8px',
                        border: '1px solid',
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                          ...
                          <Chip label={step.status} size="small" sx={{
                        height: 16,
                        fontSize: '8px',
                        fontWeight: 800,
                        ...
                        {step.expression && (<Box sx={{
                            ...
                            {step.expressionResult !== undefined && (<Box sx={{ display: 'block', mt: 0.5, fontWeight: 700, fontSize: '0.75rem', color: 'text.secondary' }}>
                                Result: {formatValue(step.expressionResult)}
                              </Box>)}
                          </Box>)}

                        {step.notes && (<Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1, fontStyle: 'italic' }}>
                            {step.notes}
                          </Typography>)}

                        {step.durationMs !== undefined && (<Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5, textAlign: 'right' }}>
            ...
            {tabValue === 1 && (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <TextField fullWidth size="small" placeholder="Filter keys..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} slotProps={{
                input: {
                ...
                {filteredContext.length === 0 ? (<Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center', mt: 2 }}>
                    No context variables found.
                  </Typography>) : (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {filteredContext.map(([key, val]) => (<Box key={key} sx={{
            ...
            {tabValue === 2 && (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                ...
                {isLoadingTasks ? (<Box sx={{ display: 'flex', justifyContent: 'center', p: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                      ...
                      {tasks.length === 0 ? (<Typography variant="body2" color="text.secondary">No task instances recorded.</Typography>) : (tasks.map((task) => (<Box key={task.id} sx={{
                        ...
                        color: task.status === 'COMPLETED' ? '#10b981' : '#f59e0b'
                            ...
                            {task.inputData && Object.keys(task.inputData).length > 0 && (<Box sx={{ mt: 0.5 }}>
                            ...
                            {task.outputData && Object.keys(task.outputData).length > 0 && (<Box sx={{ mt: 0.5 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                      ...
                      {subscriptions.length === 0 ? (<Typography variant="body2" color="text.secondary">No event subscriptions.</Typography>) : (subscriptions.map((sub) => (<Box key={sub.id} sx={{
                        p: 1.5, borderRadius: '8px',
                        border: '1px solid', borderColor: 'divider',
                        bgcolor: 'background.default',
                        display: 'flex', flexDirection: 'column', gap: 1,
                        ...
                        color: sub.status === 'ACTIVE' ? '#3b82f6' : '#9ca3af'
                            ...
                            {sub.filterAttributes && Object.keys(sub.filterAttributes).length > 0 && (<Box sx={{ mt: 0.5 }}>
        ...
        <Box sx={{ flexGrow: 1, height: '100%', position: 'relative', overflow: 'hidden' }}>
          <DesignerCanvas onRefreshWorkflows={async () => { }} onShowNotification={onShowNotification} traceMode={true} executionTrace={trace} activeTraceStep={activeTraceStep} setActiveTraceStep={setActiveTraceStep} hideHeader={true} hideTimeline={true} runtimeGraph={runtimeGraph}/>

          {/* Floating Glassmorphic Playback controls */}
          {trace.length > 0 && (<Box sx={{
                position: 'absolute',
                bottom: 24,
                left: '50%',
              ...
              <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                ...
                <IconButton onClick={() => setIsPlaying(!isPlaying)} sx={{
              ...
              <FormControl variant="standard" size="small" sx={{ minWidth: 70 }}>
                <Select value={playbackSpeed} onChange={(e) => setPlaybackSpeed(Number(e.target.value))} sx={{
