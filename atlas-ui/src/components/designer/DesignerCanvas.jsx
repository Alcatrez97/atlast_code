...
import { ReactFlow, Background, Controls, MiniMap, addEdge, useNodesState, useEdgesState, ReactFlowProvider, useReactFlow } from '@xyflow/react';
import '@xyflow/react/dist/style.css';
import { Box, Typography, Button, Chip, AppBar, Toolbar, IconButton, CircularProgress, useTheme, Select, MenuItem } from '@mui/material';
...
const getEdgeColor = (edge) => {
    const outcomeType = (edge.data?.outcomeType || '').toLowerCase();
    const label = (edge.label || '').toLowerCase();
    if (outcomeType === 'accept' || label === 'accept' || label.includes('accept') || label.includes('approved')) {
        return '#10b981'; // Green
    }
    if (outcomeType === 'reject' || label === 'reject' || label.includes('reject') || label.includes('rejected')) {
        return '#ef4444'; // Red
    }
    if (outcomeType === 'park' || label === 'park') {
        return '#f59e0b'; // Yellow/Orange
    }
    return '#818cf8'; // Default indigo
};
const DesignerCanvasInner = ({ onRefreshWorkflows, onShowNotification, traceMode = false, executionTrace = EMPTY_TRACE, activeTraceStep: propActiveTraceStep, setActiveTraceStep: propSetActiveTraceStep, hideHeader = false, hideTimeline = false, runtimeGraph = null, }) => {
    ...
    const [defaultEdgeType, setDefaultEdgeType] = useState('smoothstep');
    const [canvasKey, setCanvasKey] = useState(0);
    const [localActiveTraceStep, setLocalActiveTraceStep] = useState(null);
    ...
    useEffect(() => {
        ...
        if (hasRuntimeGraph || selectedVersion?.definition) {
            ...
            if (traceMode && executionTrace.length > 0) {
                ...
                const finalNodes = styledNodes.map((n) => {
    ...
    useEffect(() => {
        if (traceMode && activeTraceStep !== null && executionTrace[activeTraceStep]) {
            ...
            const node = rawNodes.find((n) => n.id === step.nodeId);
            if (node) {
    ...
    const saveWorkflow = async (nodesToSave, edgesToSave) => {
        if (!selectedVersion)
            return;
        setIsSaving(true);
        try {
    ...
    const handleSaveEdge = async (edgeId, updates) => {
        const updatedEdges = edges.map((e) => {
            if (e.id === edgeId) {
                const dummyEdge = { label: updates.label, data: { outcomeType: updates.outcomeType } };
                const stroke = getEdgeColor(dummyEdge);
                return {
                    ...e,
                    label: updates.label,
                    data: { ...e.data, condition: updates.condition, outcomeType: updates.outcomeType },
    ...
    const handleEdgeTypeChange = (newType) => {
        ...
        setEdges(updatedEdges);
        // Force ReactFlow to remount all edges so the new edge type visually applies.
        // ReactFlow caches edge renderers by id, so type-only changes are not reflected
        // unless we force a full canvas remount via the key prop.
        setCanvasKey((k) => k + 1);
        if (!isReadOnly) {
      ...
      <Box sx={{ display: 'flex', flexGrow: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{ flexGrow: 1, height: '100%', bgcolor: 'background.default', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <Box sx={{ flexGrow: 1, overflow: 'hidden', position: 'relative' }} onDragOver={onDragOver} onDrop={onDrop}>
            ...
            borderRadius: '8px'
        }} maskColor={theme.palette.mode === 'dark' ? 'rgba(0, 0, 0, 0.6)' : 'rgba(255, 255, 255, 0.6)'} nodeColor={() => theme.palette.primary.main}/>
            </ReactFlow>
          </Box>

          {/* Trace Timeline (Sprint 4) */}
          {traceMode && executionTrace.length > 0 && !hideTimeline && (<TraceTimeline trace={executionTrace} activeStep={activeTraceStep} onStepClick={(i) => setActiveTraceStep(activeTraceStep === i ? null : i)}/>)}
        </Box>

        {/* Node Properties Drawer */}
        <NodePropertiesDrawer open={drawerOpen && !!activeNode} node={activeNode ? {
            id: activeNode.id,
            type: activeNode.type,
            label: activeNode.data?.label,
            data: activeNode.data,
        } : null} isReadOnly={isReadOnly} traceStep={traceStep} onClose={() => setDrawerOpen(false)} onSaveNode={!isReadOnly ? handleSaveNode : undefined} onDeleteNode={!isReadOnly ? handleDeleteNode : undefined}/>

        {/* Edge Properties Drawer */}
        <EdgePropertiesDrawer open={edgeDrawerOpen && !!activeEdge} edge={activeEdge ? {
            id: activeEdge.id,
            label: activeEdge.label,
            source: activeEdge.source,
            target: activeEdge.target,
            data: activeEdge.data,
        } : null} nodes={nodes} isReadOnly={isReadOnly} onClose={() => setEdgeDrawerOpen(false)} onSaveEdge={!isReadOnly ? handleSaveEdge : undefined} onDeleteEdge={!isReadOnly ? handleDeleteEdge : undefined}/>
      </Box>
    </Box>);
};
export const DesignerCanvas = (props) => {
