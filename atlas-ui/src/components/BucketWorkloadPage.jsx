    ...
    return (<Box sx={{ bgcolor: 'background.default', minHeight: '92vh', py: 4, color: 'text.primary', transition: 'background-color 0.25s ease-in-out' }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <IconButton onClick={() => goBack()} sx={{ color: '#60a5fa', border: '1px solid rgba(96,165,250,0.3)', '&:hover': { borderColor: '#60a5fa', background: 'rgba(96,165,250,0.1)' } }}>
              <ArrowLeft size={18}/>
            </IconButton>
            <Box>
          ...
          <Tooltip title="Refresh workload data">
            <IconButton onClick={fetchWorkloads} disabled={loading} sx={{ color: 'primary.main', border: '1px solid', borderColor: 'divider', '&:hover': { background: 'action.hover' } }}>
              <RefreshCw size={18}/>
            </IconButton>
          </Tooltip>
        </Box>
        ...
        ].map((kpi) => (<Grid size={{ xs: 6, md: 3 }} key={kpi.label}>
              <Paper sx={{ p: 3, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box>
                  ...
                  {kpi.alert && kpi.value > 0 && (<Typography variant="caption" sx={{ display: 'block', color: '#f87171', fontSize: '0.65rem' }}>⚠ SLA exceeded!</Typography>)}
                </Box>
              </Paper>
            </Grid>))}
        </Grid>

        {totalSlaBreached > 0 && (<Alert severity="error" sx={{ mb: 3, borderRadius: 2, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)' }}>
        ...
        {/* Bucket Cards */}
        {workloads.length === 0 && !loading ? (<Paper sx={{ p: 6, textAlign: 'center', background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(96,165,250,0.1)', borderRadius: 3 }}>
          ...
          </Paper>) : (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {workloads.map((w) => {
                ...
                return (<Paper key={w.bucketId} sx={{ bgcolor: 'background.paper', border: `1px solid ${isExpanded ? priorityColor : 'divider'}`, borderRadius: 2, overflow: 'hidden', transition: 'border-color 0.2s' }}>
                  {/* Bucket header row */}
                  <Box sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer', '&:hover': { background: 'rgba(255,255,255,0.02)' } }} onClick={() => toggleBucket(w.bucketId)}>
                    {/* Priority indicator */}
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
                        <Typography variant="subtitle1" sx={{ color: 'text.primary', fontWeight: 500 }}>{w.bucketName}</Typography>
                        <Chip label={w.priority} size="small" sx={{ background: priorityBg, color: priorityColor, border: `1px solid ${priorityColor}44`, fontWeight: 500, fontSize: '0.65rem' }}/>
                        {w.slaHours && (<Chip icon={<Timer size={12}/>} label={`SLA: ${w.slaHours}h`} size="small" sx={{ background: 'action.hover', color: 'text.secondary', fontSize: '0.65rem' }}/>)}
                        {w.ownerGroup && (<Chip icon={<Users size={12}/>} label={w.ownerGroup} size="small" sx={{ background: 'action.hover', color: 'text.secondary', fontSize: '0.65rem' }}/>)}
                      </Box>
                      <Typography variant="caption" sx={{ color: '#475569', fontFamily: 'monospace' }}>{w.bucketId}</Typography>
                      {/* Resolution progress bar */}
                      {w.totalRouted > 0 && (<Box sx={{ mt: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2, flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    ...
                    ].map((s) => (<Box key={s.label} sx={{ textAlign: 'center', minWidth: 44 }}>
                      ...
                      {w.slaBreached > 0 && (<Box sx={{ textAlign: 'center', minWidth: 44 }}>
                  ...
                  <Collapse in={isExpanded}>
                    ...
                    <Box sx={{ p: 2 }}>
                      {loadingItems === w.bucketId ? (<Box sx={{ py: 3, textAlign: 'center' }}><CircularProgress size={24}/></Box>) : !bucketItems[w.bucketId] || bucketItems[w.bucketId].length === 0 ? (<Typography sx={{ color: '#475569', textAlign: 'center', py: 3 }}>No executions routed to this bucket yet.</Typography>) : (<TableContainer>
                          <Table size="small">
                            <TableHead>
                              <TableRow>
                                {['Workflow', 'Status', 'SLA', 'Routed At', 'Resolved At', 'Resolved By', 'Resolution Trace'].map(col => (<TableCell key={col} sx={{ color: 'text.secondary', borderBottom: '1px solid', borderColor: 'divider', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: 0.5 }}>{col}</TableCell>))}
                              </TableRow>
                            </TableHead>
                            <TableBody>
                              {bucketItems[w.bucketId].map((item) => (<TableRow key={item.id} onClick={() => fetchExecutionDetails(item)} sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' } }}>
                                  <TableCell sx={{ color: 'text.primary', borderBottom: '1px solid', borderColor: 'divider', fontFamily: 'monospace', fontSize: '0.75rem' }}>
                                    {item.workflowKey}
                                  </TableCell>
                                ...
                                : <Typography sx={{ color: 'text.secondary', fontSize: '0.75rem' }}>OK</Typography>}
                                  ...
                                  <TableCell sx={{ color: 'text.secondary', borderBottom: '1px solid', borderColor: 'divider', fontSize: '0.75rem' }}>
                      ...
                      {w.avgResolutionHours != null && (<Box sx={{ mt: 2, px: 1, display: 'flex', gap: 1, alignItems: 'center' }}>
      ...
      <Dialog open={detailsModalOpen} onClose={() => setDetailsModalOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 2, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          {selectedExecution && (<Chip label={selectedExecution.status} size="small" color={STATUS_CHIP_COLOR[selectedExecution.status] ?? 'default'} sx={{ fontWeight: 700, fontSize: '0.7rem' }}/>)}
        </DialogTitle>

        <DialogContent sx={{ py: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
          {modalLoading ? (<Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 8 }}>
