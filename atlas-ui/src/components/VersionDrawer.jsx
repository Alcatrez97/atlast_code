...
export const VersionDrawer = ({ open, onClose, onCreateDraft, onTransitionStatus, onDeleteVersion }) => {
    const { selectedWorkflow, activeRole, setSelectedVersion, setView } = useWorkflowStore();
    const [errorMsg, setErrorMsg] = useState(null);
    if (!selectedWorkflow)
        return null;
    const handleOpenDesigner = (version) => {
    ...
    return (<Drawer anchor="right" open={open} onClose={onClose} slotProps={{ paper: { sx: { width: 450, bgcolor: 'background.paper', borderLeft: '1px solid', borderColor: 'divider' } } }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Box sx={{ flexGrow: 1, overflow: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
          {errorMsg && (<Alert severity="error" onClose={() => setErrorMsg(null)} sx={{ borderRadius: 2 }}>
              {errorMsg}
            </Alert>)}

          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, color: 'text.primary' }}>Version List</Typography>
            <Button variant="outlined" size="small" startIcon={<AddIcon />} onClick={onCreateDraft} sx={{ borderColor: 'rgba(44, 42, 42, 1)', color: 'text.primary' }}>
              New Draft
            </Button>
          </Box>

          {sortedVersions.length === 0 ? (<Box sx={{ textAlign: 'center', py: 6, border: '1px dashed rgba(7, 7, 7, 0.43)', borderRadius: 4 }}>
              <Typography color="text.secondary">No versions available. Create a draft version to start.</Typography>
            </Box>) : (<Stepper orientation="vertical" connector={null} sx={{ p: 0 }}>
              {sortedVersions.map((ver) => {
                ...
                return (<Step key={ver.id} active expanded>
                    <StepLabel icon={<Box sx={{
                            width: 12,
                            height: 12,
                            borderRadius: '50%',
                        ...
                        <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                          ...
                          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                            {ver.status.toUpperCase() === 'DRAFT' && (<>
                                <Button size="small" variant="contained" color="warning" onClick={() => handleTransition(ver, 'REVIEW')} startIcon={<RateReviewIcon sx={{ fontSize: 12 }}/>}>
                                  Submit Review
                                </Button>
                                <Button size="small" variant="outlined" sx={{ color: 'text.primary' }} onClick={() => handleOpenDesigner(ver)} startIcon={<EditIcon sx={{ fontSize: 12 }}/>}>
                                  Edit Design
                                </Button>
                              </>)}

                            {ver.status.toUpperCase() === 'REVIEW' && (<>
                                <Button size="small" variant="contained" color="success" onClick={() => handleTransition(ver, 'APPROVED')} startIcon={<CheckCircleOutlinedIcon />}>
                                  Approve
                                </Button>
                                <Button size="small" variant="outlined" color="error" onClick={() => handleTransition(ver, 'DRAFT')}>
                                  Reject
                                </Button>
                                <Button size="small" variant="outlined" color="info" onClick={() => handleOpenDesigner(ver)} startIcon={<VisibilityIcon sx={{ fontSize: 12 }}/>}>
                                  View Design
                                </Button>
                              </>)}

                            {ver.status.toUpperCase() === 'APPROVED' && (<>
                                <Button size="small" variant="contained" color="secondary" onClick={() => handleTransition(ver, 'PUBLISHED')} startIcon={<LockIcon />}>
                                  Publish Active
                                </Button>
                                <Button size="small" variant="outlined" color="warning" onClick={() => handleTransition(ver, 'DRAFT')}>
                                  Revert Draft
                                </Button>
                                <Button size="small" variant="outlined" color="info" onClick={() => handleOpenDesigner(ver)} startIcon={<VisibilityIcon sx={{ fontSize: 12 }}/>}>
                                  View Design
                            ...
                            {!isPublished && (<Button size="small" color="error" onClick={() => onDeleteVersion(ver.id)} sx={{ ml: 'auto' }}>
                                Delete
                              </Button>)}
                          </Box>
                        </CardContent>
