...
export const TraceTimeline = ({ trace, activeStep, onStepClick }) => {
        ...
        {trace.map((step, i) => {
            ...
            return (<React.Fragment key={i}>
              ...
              <Tooltip title={<Box>
                    ...
                    {step.expression && (<Typography variant="caption" sx={{ display: 'block', fontFamily: 'monospace', color: '#818cf8', mt: 0.5 }}>
                        {step.expression} → {JSON.stringify(step.expressionResult)}
                      </Typography>)}
                  </Box>} placement="top">
                <Box onClick={() => onStepClick(i)} sx={{
                    display: 'flex',
                  ...
                  {/* Duration */}
                  {step.durationMs !== undefined && (<Typography variant="caption" sx={{ fontSize: '8px', color: 'text.secondary' }}>
                      {step.durationMs}ms
                    </Typography>)}
                </Box>
              </Tooltip>
