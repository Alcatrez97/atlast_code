...
import { Box, Typography, TextField, Button, Switch, FormControlLabel, CircularProgress, IconButton, Divider, Alert, Paper, Container } from '@mui/material';
import ArrowBackIcon from '@mui/icons-material/ArrowBack';
import SaveIcon from '@mui/icons-material/Save';
import CheckCircleIcon from '@mui/icons-material/CheckCircle';
import ErrorIcon from '@mui/icons-material/Error';
export const RuleForm = ({ rule, onClose, onRefresh, onShowNotification }) => {
    ...
    return (<Container maxWidth="md" sx={{ py: 2 }}>
      ...
      <Paper component="form" onSubmit={handleSubmit} elevation={0} sx={{
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <FormControlLabel control={<Switch checked={active} onChange={(e) => setActive(e.target.checked)} color="secondary"/>} label={active ? 'rule active' : 'rule suspended'} sx={{ '& .MuiFormControlLabel-label': { fontWeight: 700 } }}/>
        </Box>

        {/* Rule Key */}
        <TextField fullWidth size="small" label="Rule Business Key" placeholder="RULE_HIGH_VAL" value={ruleKey} onChange={(e) => setRuleKey(e.target.value)} disabled={!!rule} // Key is immutable after registration
     helperText="uppercase unique identifier used in flow diagram configuration" slotProps={{ input: { sx: { color: 'text.primary', fontFamily: 'monospace' } } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: 'divider' } }}/>
        ...
        {/* Name */}
        <TextField fullWidth size="small" label="Rule Display Name" placeholder="High Value Amount Check" value={name} onChange={(e) => setName(e.target.value)} slotProps={{ input: { sx: { color: 'text.primary' } } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: 'divider' } }}/>

        {/* Description */}
        <TextField fullWidth size="small" label="Description" placeholder="Checks if the transaction amount is greater than 10,000" value={description} onChange={(e) => setDescription(e.target.value)} multiline rows={3} slotProps={{ input: { sx: { color: 'text.primary' } } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: 'divider' } }}/>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
            ...
            <Button variant="outlined" size="small" onClick={handleValidate} disabled={validating} sx={{
          ...
          <TextField fullWidth multiline rows={4} placeholder="context['amount'] > 10000" value={expression} onChange={(e) => {
            setExpression(e.target.value);
            setValidationResult(null);
        }} slotProps={{ input: { sx: { color: 'text.primary', fontFamily: 'monospace', fontSize: '13px' } } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: 'divider' } }}/>
          <Typography variant="caption" color="text.secondary">
            Write expressions in terms of <code>context['key']</code>, e.g., <code>context['amount'] &gt; 5000</code>.
          </Typography>

          {/* Validation Banner */}
          {validationResult && (<Alert severity={validationResult.valid ? 'success' : 'error'} icon={validationResult.valid ? <CheckCircleIcon /> : <ErrorIcon />} sx={{
                borderRadius: 2,
                fontSize: '12px',
                py: 0.5,
