...
import com.vi.atlas.workflow.service.BucketExecutionService;
...
public class BucketExecutionController {
    ...
    @GetMapping("/workload")
    @Operation(summary = "Get all workload metrics", description = "Retrieves operational workload summary KPIs across all buckets, optionally filtered by circleId")
    public ResponseEntity<List<BucketWorkloadDto>> getAllWorkload(
            @RequestParam(required = false) String circleId) {
        return ResponseEntity.ok(bucketExecutionService.getAllWorkloadSummaries(circleId));
    }
