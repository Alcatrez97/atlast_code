...
import com.vi.atlas.workflow.service.traversal.node.NodeExecutor;
import com.vi.atlas.workflow.service.traversal.node.NodeExecutorRegistry;
...
/**
 * Helper component that contains the sequential (pointerâ€‘based) traversal algorithm.
...
public class SequentialEngine {
    ...
    public GraphTraversalEngine.TraversalResult runSequentialTraversal(
        ...
        while (!activeFrontiers.isEmpty() && stepIdx[0] < MAX_STEPS) {
            ...
            // Cycle detection (skip for JOIN nodes which must be reâ€‘visited)
            if (!"JOIN".equalsIgnoreCase(currentNode.getType())) {
                if (visited.contains(currentNode.getId())) {
                    trace.add(taskRecorder.buildStep(stepIdx[0]++, currentNode, "SKIPPED",
                            null, null, null, "Cycle detected â€“ node already visited; halting."));
                    continue;
                }
