 ...
 * <ul>
 *   <li>{@code SUB_WORKFLOW} â€” apply the child's output mapping back to parent context</li>
 *   <li>{@code WAIT_EVENT}   â€” resolve which node to jump to based on routed value/routes</li>
 *   <li>{@code COMMAND} (ASYNC mode) â€” mark node active, record RESUMED step, continue</li>
 *   <li>Generic / BUCKET     â€” choose edge by outcome and advance the frontier</li>
 * </ul>
 *
 * <p>All logic is copied verbatim from the corresponding blocks inside
 * {@code GraphTraversalEngine#traverse()} â€” zero algorithmic changes.
 */
@Component
public class ResumeRouter {
     ...
     * (which causes the loop to start from the START node instead â€” should not
     * happen in practice).
     *
    ...
    public WorkflowNodeDto resolveSequentialResumeNode(String startNodeId,
        ...
        switch (nodeType) {
            ...
            case "SUB_WORKFLOW" -> {
                ...
                // fall through â€” the edge is chosen below (currentNode == null path)
            }
     ...
     * traversal pass</em> â€” used by the event routing layer when it can deliver
    ...
    public static boolean trySynchronousResumption(String instanceId,
        ...
        if (payload != null) {
            applyPayloadMapping(suspendedNode.getData(), payload, travCtx.context);
        }
    ...
    /**
     * Applies selective payload mapping from nodeData to targetContext.
     * If nodeData contains a 'payloadMapping' map (sourcePayloadKey -> targetContextKey),
     * only mapped fields are extracted. Otherwise, the full payload map is merged.
     */
    public static void applyPayloadMapping(Map<String, Object> nodeData,
                                            Map<String, Object> payload,
                                            Map<String, Object> targetContext) {
        if (payload == null || targetContext == null) return;
        Object mappingObj = nodeData != null ? nodeData.get("payloadMapping") : null;
        if (mappingObj instanceof Map) {
            ...
            for (Map.Entry<?, ?> entry : mapping.entrySet()) {
                ...
                Object val = payload.get(sourceKey);
                if (val != null) {
                    targetContext.put(targetKey, val);
                }
            }
        } else {
            // Default backward compatible behavior: merge all payload entries
            targetContext.putAll(payload);
        }
    }
}
