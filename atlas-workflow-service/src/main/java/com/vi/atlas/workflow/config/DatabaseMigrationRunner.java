...
public class DatabaseMigrationRunner {
    ...
    private boolean columnExists(Statement stmt, String tableName, String colName) throws Exception {
        String query = String.format(
                "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS " +
                "WHERE UPPER(TABLE_NAME) = '%s' AND UPPER(COLUMN_NAME) = '%s'",
                tableName.toUpperCase(), colName.toUpperCase()
        );
        try (ResultSet rs = stmt.executeQuery(query)) {
            return rs.next() && rs.getInt(1) > 0;
        }
    ...
    private void alterColumnTypeToIntegerIfExist(Statement stmt, String tableName, String colName) throws Exception {
        if (tableExists(stmt, tableName) && columnExists(stmt, tableName, colName)) {
            try {
                // If non-numeric string data exists, nullify it before converting column to INTEGER
                stmt.execute(String.format("UPDATE %s SET %s = NULL WHERE %s IS NOT NULL AND %s NOT REGEXP '^[0-9]+$'", tableName, colName, colName, colName));
            } catch (Exception ignored) {}
            try {
                stmt.execute(String.format("ALTER TABLE %s ALTER COLUMN %s INTEGER", tableName, colName));
                log.info("Successfully altered {}.{} to INTEGER.", tableName, colName);
            } catch (Exception e) {
                log.debug("Could not alter {}.{} to INTEGER: {}", tableName, colName, e.getMessage());
            }
        }
    }
