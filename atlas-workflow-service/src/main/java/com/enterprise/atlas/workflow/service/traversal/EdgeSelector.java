 ...
 * <ol>
 *   <li>{@link #chooseEdge} — for RULE nodes (true/false condition on each edge)</li>
 *   <li>{@link #matchDecisionEdge} — for DECISION nodes (value equality or SpEL on each edge)</li>
 *   <li>{@link #chooseBucketEdgeByOutcome} — for BUCKET resume (outcomeType field on each edge)</li>
 *   <li>{@link #isBucketEdgeTaken} — used by the activation-based traversal to check
 *       whether an individual edge from a completed BUCKET node was actually traversed</li>
 * </ol>
 *
 * <p>All logic is copied verbatim from {@code GraphTraversalEngine} — no algorithmic changes.
 */
@Component
public class EdgeSelector {
     ...
     * <ol>
     *   <li>Edge has a matching {@code outcomeType} → yes</li>
     *   <li>Edge has no {@code outcomeType} but another edge does match → no</li>
     *   <li>Edge has an arbitrary SpEL condition → evaluate it</li>
