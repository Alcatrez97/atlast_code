 ...
 * <ul>
 *   <li>{@code nextNode} — next node to enqueue in the frontier (null = nothing follows)</li>
 *   <li>{@code edgeTakenId} — ID of the edge used to reach {@code nextNode} (for runtime graph)</li>
 *   <li>{@code suspended} — if {@code true} the engine must pause the traversal loop</li>
 *   <li>{@code continueLoop} — if {@code true} use {@code continue} instead of the normal
