...
import { Box, Container, Grid, Paper, Typography, Button, Card, CardContent, Chip, IconButton, InputBase, CircularProgress, Divider, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Collapse, Tooltip, FormControl, Select, MenuItem } from '@mui/material';
...
export const WorkflowInstancesPage = ({ onShowNotification }) => {
  const { setView, setCurrentExecution, setReplayVersion, workflows, goBack, showConfirm, selectedCircleIds } = useWorkflowStore();
  ...
  const fetchInstancesAndExecutions = async (targetPage = page, targetSize = size, wfKey = selectedWorkflowKey, status = selectedStatus, searchTerm = search, circles = selectedCircles) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    fetchInstancesAndExecutions(page, size, selectedWorkflowKey, selectedStatus, search, selectedCircles);
  }, [page, size, selectedWorkflowKey, selectedStatus, selectedCircles]);
  ...
  const handleDeleteInstance = async (id, e) => {
    ...
    const isConfirmed = await showConfirm({
      ...
      message: 'Are you sure you want to delete this stateful workflow instance? This action cannot be undone.',
  ...
  const getStatusChip = (status) => {
    ...
    return (<Chip label={status} size="small" icon={icon} sx={{
  ...
  return (<Box className="atlas-instances-root" sx={{ bgcolor: 'background.default', minHeight: '92vh', py: 4, color: 'text.primary', transition: 'background-color 0.25s ease-in-out, color 0.25s ease-in-out' }}>
    <Container className="atlas-instances-container" maxWidth={false} sx={{ px: { xs: 2, sm: 3, md: 4 } }}>
      {/* Title bar */}
      <Box className="atlas-instances-title-bar" sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', mb: 4, gap: 2 }}>
        <Box className="atlas-instances-title-group" sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <IconButton className="atlas-instances-back-btn" onClick={() => goBack()} sx={{ color: '#2F3043', border: '1px solid #2f304344' }}>
            <ArrowBackIcon />
          </IconButton>
          <Box className="atlas-instances-title-text">
            <Typography className="atlas-instances-title" variant="h4" sx={{ fontWeight: 800, letterSpacing: '-0.5px' }}>
              Workflow Instances
            </Typography>
            <Typography className="atlas-instances-subtitle" variant="body2" color="text.secondary">
              Monitor stateful transaction loops, inspect lazy variables context, and resume waiting processes.
            </Typography>
          </Box>
        </Box>
        <IconButton className="atlas-instances-refresh-btn" onClick={() => fetchInstancesAndExecutions()} sx={{ color: accentColor, border: `1px solid rgba(99,102,241,0.2)` }}>
          <RefreshIcon />
        </IconButton>
      </Box>

      {/* Filter Panel */}
      <Paper className="atlas-instances-filter-paper" sx={{
        p: 3, mb: 4,
        ...
        <Grid container spacing={2} sx={{ alignItems: 'center' }}>
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            ...
            <Box sx={{
              ...
              <InputBase placeholder="ID, key, or current node..." value={search} onChange={(e) => {
          ...
          <Grid size={{ xs: 12, sm: 4, md: 3 }}>
            ...
            <FormControl fullWidth size="small">
              <Select value={selectedWorkflowKey} onChange={(e) => {
                ...
                {workflows.map(w => (<MenuItem key={w.key} value={w.key} sx={{ fontSize: '14px' }}>
                  {w.name} ({w.key})
                </MenuItem>))}
              </Select>
            </FormControl>
      ...
      {/* Loading / Content */}
      {loading && instances.length === 0 ? (<Box sx={{ display: 'flex', justifyContent: 'center', py: 12 }}>
        <CircularProgress color="secondary" />
      </Box>) : instances.length === 0 ? (<Box sx={{
        ...
        {instances.map((inst) => {
          ...
          return (<Grid size={{ xs: 12 }} key={inst.id}>
            <Card sx={{
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box sx={{ flexGrow: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                      ...
                      {inst.circleId != null && (() => {
                        const opt = CIRCLE_OPTIONS.find(c => c.id === inst.circleId);
                        return <Chip label={opt ? opt.label : `Circle ${inst.circleId}`} size="small" variant="outlined" sx={{ height: 22, fontSize: '11px', borderColor: opt ? opt.color : 'rgba(99,102,241,0.4)', color: opt ? opt.color : '#a5b4fc', fontWeight: 700 }} />;
                      })()}
                    ...
                    {inst.currentNodeId && (<Typography variant="body2" sx={{ color: isWaiting ? '#f59e0b' : 'text.secondary', fontWeight: 600 }}>
                ...
                <Collapse in={isExpanded} timeout="auto" unmountOnExit sx={{ mt: 3 }}>
                  ...
                  <Grid container spacing={4}>
                    ...
                    <Grid size={{ xs: 12, md: 4 }}>
                      ...
                      {loadingRevert[inst.id] ? (<Box sx={{ display: 'flex', justifyContent: 'center', py: 6 }}>
                        <CircularProgress size={20} color="secondary" />
                      </Box>) : !revertLogs[inst.id] || revertLogs[inst.id].length === 0 ? (<Typography variant="body2" color="text.secondary" sx={{ fontStyle: 'italic', py: 2 }}>
                        No external bucket approvals or revert entries logged.
                      </Typography>) : (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, maxHeight: 300, overflowY: 'auto', pr: 0.5 }}>
                        {revertLogs[inst.id].map((log) => {
                          ...
                          return (<Paper key={log.id} sx={{
                            ...
                            {log.previousStepId && (<Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontSize: '10px' }}>
                              Chained previous step ID: <span style={{ fontFamily: 'monospace' }}>{log.previousStepId.substring(0, 8)}...</span>
                            </Typography>)}

                            {deps.length > 0 && (<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
                              <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: '10px' }}>Deps:</Typography>
                              {deps.map((dep, dIdx) => (<Chip key={dIdx} label={dep} size="small" sx={{ height: 16, fontSize: '8px', bgcolor: 'rgba(99,102,241,0.1)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.2)' }} />))}
                            </Box>)}

                            {log.resolvedBy && (<Box sx={{ mt: 0.5, p: 0.8, bgcolor: 'background.default', borderRadius: 1 }}>
                              <Typography variant="caption" sx={{ display: 'block', color: 'text.secondary', fontSize: '10px' }}>
                                Resolved by: <strong>{log.resolvedBy}</strong>
                              </Typography>
                              {log.resolutionNotes && (<Typography variant="caption" sx={{ color: '#cbd5e1', display: 'block', fontStyle: 'italic', mt: 0.5, fontSize: '10px' }}>
                    ...
                    <Grid size={{ xs: 12, md: 4 }}>
                      ...
                      {relatedRuns.length === 0 ? (<Typography variant="body2" color="text.secondary">
                        No logged segment runs recorded.
                      </Typography>) : (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, maxHeight: 300, overflowY: 'auto' }}>
                        {relatedRuns.map((run, idx) => (<Paper key={run.id} sx={{
      ...
      {/* Pagination Bar */}
      {totalElements > 0 && (<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 4, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          <Button variant="outlined" size="small" disabled={page === 0} onClick={() => setPage(prev => Math.max(0, prev - 1))} sx={{ borderRadius: 1.5, borderColor: 'divider', fontWeight: 700 }}>
            Previous
          </Button>
          {Array.from({ length: totalPages }).map((_, idx) => {
            ...
            return (<Button key={idx} variant={isSelected ? 'contained' : 'outlined'} size="small" onClick={() => setPage(idx)} sx={{
              minWidth: 32,
              width: 32,
              height: 32,
              p: 0,
          ...
          <Button variant="outlined" size="small" disabled={page >= totalPages - 1} onClick={() => setPage(prev => Math.min(totalPages - 1, prev + 1))} sx={{ borderRadius: 1.5, borderColor: 'divider', fontWeight: 700 }}>
            Next
          </Button>
        </Box>
      </Box>)}
