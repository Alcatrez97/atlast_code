...
export const ValidationResultBanner = ({ validation }) => {
    if (!validation)
        return null;
    ...
    if (!hasErrors && !hasWarnings)
        return null;
    return (<Box sx={{ m: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      {hasErrors && (<Alert severity="error" icon={<ErrorIcon />} sx={{
                borderRadius: 2,
      ...
      {hasWarnings && (<Alert severity="warning" icon={<WarningAmberIcon />} sx={{
                borderRadius: 2,
