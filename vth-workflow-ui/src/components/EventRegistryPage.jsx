...
import { Box, Container, Grid, Paper, Typography, Button, Card, CardContent, CardActions, Chip, IconButton, InputBase, CircularProgress, Divider, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from '@mui/material';
...
export const EventRegistryPage = ({ onShowNotification }) => {
    ...
    const handleOpenSimulation = (event) => {
    ...
    const handleRunSimulation = async () => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('Failed to ingest event simulation payload');
    ...
    return (<Box sx={{ bgcolor: 'background.default', minHeight: '92vh', py: 4, color: 'text.primary', transition: 'background-color 0.25s ease-in-out' }}>
      <Container maxWidth={false} sx={{ px: { xs: 2, sm: 3, md: 4 } }}>
        ...
        {viewMode !== 'list' ? (<EventForm event={selectedEvent} onClose={() => setViewMode('list')} onRefresh={fetchEvents} onShowNotification={onShowNotification}/>) : (<>
            {/* Search bar */}
            <Paper sx={{
              ...
              <InputBase placeholder="Search events by Key, Name, Description, or Topic..." value={search} onChange={(e) => setSearch(e.target.value)} sx={{ ml: 1, flex: 1, color: 'text.primary', fontSize: '14px' }}/>
            </Paper>

            {/* Loading / Content */}
            {loading && events.length === 0 ? (<Box sx={{ display: 'flex', justifyContent: 'center', py: 12 }}>
                <CircularProgress color="secondary"/>
              </Box>) : filteredEvents.length === 0 ? (<Box sx={{
                ...
                {filteredEvents.map((ev) => {
                    return (<Grid size={{ xs: 12, sm: 6, md: 4 }} key={ev.id}>
                      <Card sx={{
                        ...
                        <CardContent sx={{ flexGrow: 1, p: 2.5 }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
                            ...
                            {/* Status Chip */}
                            <Chip label={ev.active ? "ACTIVE" : "INACTIVE"} size="small" sx={{
                          ...
                          {ev.payloadSchema && (<Box sx={{ bgcolor: 'rgba(0,0,0,0.2)', p: 1.5, borderRadius: 1.5, border: '1px solid rgba(255,255,255,0.04)', mb: 2 }}>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            {ev.active ? (<ToggleOnIcon sx={{ color: '#22c55e', fontSize: 16 }}/>) : (<ToggleOffIcon sx={{ color: '#ef4444', fontSize: 16 }}/>)}
                        ...
                        <CardActions sx={{ justifyContent: 'space-between', px: 2.5, pb: 2.5, pt: 0 }}>
                          <Button size="small" variant="outlined" startIcon={<PlayArrowIcon />} onClick={() => handleOpenSimulation(ev)} sx={{
                            borderColor: accentColor,
                            color: accentColor,
                            fontWeight: 700,
                            borderRadius: 2,
        ...
        {/* Simulation Dialog */}
        <Dialog open={simulationOpen} onClose={() => setSimulationOpen(false)} fullWidth maxWidth="sm" slotProps={{ paper: { sx: { bgcolor: 'background.paper', color: 'text.primary', borderRadius: 2 } } }}>
          ...
          <DialogContent>
            ...
            <TextField fullWidth multiline rows={12} label="Standard Event JSON Payload" value={simulationPayload} onChange={(e) => setSimulationPayload(e.target.value)} slotProps={{ input: { sx: { fontFamily: 'monospace', fontSize: '13px', color: 'text.primary' } } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { borderColor: 'divider' } }}/>
          </DialogContent>
          <DialogActions sx={{ p: 2.5, gap: 1 }}>
            <Button onClick={() => setSimulationOpen(false)} color="inherit" sx={{ textTransform: 'lowercase', fontWeight: 700 }}>
              cancel
            </Button>
            <Button onClick={handleRunSimulation} variant="contained" disabled={simulating} startIcon={simulating ? <CircularProgress size={14} color="inherit"/> : <PlayArrowIcon />} sx={{
